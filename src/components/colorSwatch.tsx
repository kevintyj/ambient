...
type IColorSwatchProps<T = object> = Component<T & {
	...
	return (
		<>
			<div class="flex h-9 items-center w-full">
				...
				<For each={Object.keys(Object.values(watchingSwatch())[0])}>
					{(id, _i) => (
			...
			<For each={Object.entries(watchingSwatch())}>
				{([color, swatch], j) => (
					<div class="flex items-center">
						<div class="w-24 pr-4 flex-none justify-end text-slate-600 dark:text-neutral-400 capitalize hidden sm:block">
							{color.toLocaleLowerCase()}
						</div>
						<For each={Object.values(swatch)}>
							{(color, k) => (
								<ColorIdentifier
									color={color}
									textColor={calcMinAPCA(Object.values(swatch), color)[1].toString()}
									tabindex={0}
