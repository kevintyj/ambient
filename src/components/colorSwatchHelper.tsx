...
type IColorSwatchHelperProps<T = object> = Component<T & {
...
const ColorSwatchHelper: IColorSwatchHelperProps = (props) => {
	const helperMod = props.activeHelper;
	...
	return (
		<>
			<div class="flex flex-col w-full">
				<div class="flex">
					<For each={props.track === 'neutral' ? baseNeutral() : baseSwatch()}>
						{(color, k) => (
							<ColorIdentifier
								color={color}
                ...
                group"
									>
										...
										<div class="bottom-12 absolute hidden group-hover:block">
											<HelperBadge active>
												{props.activeHelper ? modifiedShift(helperMod || props.activeHelper) : `0${k()}`}
											</HelperBadge>
										</div>
									</div>
								</Show>
