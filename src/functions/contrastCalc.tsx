...
export const calcAPCA = (txt: string, bg: string): number => {
	const textHex: number = Number.parseInt(`0x${txt.substring(1)}`, 16);
	const bgHex: number = Number.parseInt(`0x${bg.substring(1)}`, 16);
...
export const calcMinAPCA = (text: Array<string>, bg: string): [number, string] => {
	for (const textColor of text) {
		if (calcAPCA(textColor, bg) > (darkMode() ? 30 : 40))
			return [calcAPCA(darkMode() ? text[2] : text[8], bg), darkMode() ? text[2] : text[8]];
		if (calcAPCA(textColor, bg) < (darkMode() ? -60 : -50))
			return [calcAPCA(darkMode() ? text[8] : text[2], bg), darkMode() ? text[8] : text[2]];
	}
	return [0, '#000'];
};
