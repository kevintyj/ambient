...
export function colorParsley(colorIn) {
	if (typeof colorIn === 'string') {
		...
		if (Array.isArray(colorIn)) {
			...
			const objArray = [0, 0, 0, 0, false, 'unknown'];
...
function parseString(colorString = '#abcdef') {
	...
	const type = 'sRGB'; // Default colorspace flag in element [5]
	const retArray = [0, 0, 0, 0, isValid, type]; // init the return array
		...
		for (const key in namedColors) {
			if (colorString === key) {
				const hexRex = {
					...
					sprig(slices) {
						for (let i = 0; i < 3; i++)
							retArray[i] = Number.parseInt(slices[i + 1], 16);
				...
				const hexProc = hexRex.rex.exec(namedColors[key]);
	...
	const colorRex = {
		...
		parsley(slices) {
			...
			retArray[3] = (alpha.match(/%/g))
				? Number.parseFloat(alpha) / divisor
			...
			switch (sliceLast) {
				case 4: // This is the 3-4 digit hex parsing
					base = 16;
					divisor = 15.0;
					retArray[3] = Number.parseInt(slices[sliceLast], base) / divisor;
				case 3:
					base = 16;
					for (let i = 0; i < 3; i++)
						retArray[i] = Number.parseInt(slices[slicePos + i] + slices[slicePos + i], base);
        ...
        = (base === 10)
							? Number.parseFloat(slices[sliceLast])
							: Number.parseInt(slices[sliceLast], base);
					break;

				case 12: // allows single percentage to become grey
					retArray[0] = retArray[1] = retArray[2]
         = Number.parseFloat(slices[sliceLast]) * convertPct;
					break;

				case 8: // These are the main parsings for hex and rgb()
					base = 16;
					divisor = 255.0;
					retArray[3] = Number.parseInt(slices[8], base) / divisor;
				case 7:
					base = 16;
				case 11:
					for (let i = 0; i < 3; i++) {
						retArray[i] = (base === 10)
							? Number.parseFloat(slices[slicePos + i])
							: Number.parseInt(slices[slicePos + i], base);
					}
					break;

				case 14: //  rgb() percentage
					for (let i = 0; i < 3; i++)
						retArray[i] = Number.parseFloat(slices[slicePos + i]) * convertPct;
				...
				case 18: // This is for color() CSS 4
					...
					for (let i = 0; i < 3; i++) { // color() is converted to 0.0-255.0
						...
						retArray[i] = (slices[slicePos].match(/%/g))
							? Number.parseFloat(slices[slicePos]) * 2.55
							: Number.parseFloat(slices[slicePos]) * 255;
					}
					break;
					...
					for (let i = 0; i < 3; i++) {
						...
						retArray[i] = (slices[slicePos])
							? (slices[slicePos].match(/%/g))
									? Number.parseFloat(slices[slicePos]) / divisor
									: Number.parseFloat(slices[slicePos])
							: 0.0;
					}
	...
	//  The main call
	const slicesProc = colorRex.rex.exec(colorString);
...
function colorToHex(rgba = [0, 0, 0, ''], allow3 = true) {
	...
	if (allow3
		&& Number.parseInt(A, 16) % 17 === 0
		&& Number.parseInt(A, 16) % 17 === 0
		&& Number.parseInt(A, 16) % 17 === 0
		&& (Number.parseInt(A, 16) % 17 === 0 || A === '')
	)
